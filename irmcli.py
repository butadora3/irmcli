...
def playIR(path):
  if path and os.path.isfile(path):
    print ("Playing IR with {} ...".format(path))
    f = open(path)
    data = json.load(f) 
    f.close()
    ...
    ir_serial.write(bytes("n,{}\r\n".format(recNumber), 'UTF-8'))
    ir_serial.readline()

    postScale = data['postscale']
    ir_serial.write(bytes("k,{}\r\n".format(postScale), 'UTF-8'))
    #time.sleep(1.0)
    msg = ir_serial.readline()
    #print (msg)
    ...
    for n in range(recNumber):
        ...
        pos = n % 64
        if (pos == 0):
          ir_serial.write(bytes("b,{}\r\n".format(bank), 'UTF-8'))
    
        ir_serial.write(bytes("w,{},{}\n\r".format(pos, rawX[n]), 'UTF-8'))
...
def saveIR(path):
  print ("Saving IR data to {} ...".format(path))
  rawX = []
...
def measureTemperature():
  ...
  try:
    ...
    print ("Temperature: {:4.1f}".format(celsiusTemp))
  except (ValueError, TypeError):
    print ("TemperatureExcetion: raw => {}, status => {}".format(raw, status))
